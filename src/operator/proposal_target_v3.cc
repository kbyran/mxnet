 ...
 * \file proposal_target_v3.cc
 * \brief C++ version proposal target
 * \author Yuntao Chen, Zehao Huang, Ziyang Zhou, Bo Ke
 */
#include "./proposal_target_v3-inl.h"
#include <algorithm>
#include <cstdio>
using std::min;
using std::max;
...
inline void SampleROI(const Tensor<cpu, 2, DType> &all_rois,
  ...
  */
    /*
    printf("gt_boxes: \n");
    for (index_t i = 0; i < gt_boxes.size(0); ++i) {
        printf("[");
        for (index_t j = 0; j < 5; ++j) {
            printf("%f, ", gt_boxes[i][j]);
        }
        printf("]\n");
    }
    printf("gt_boxes_ignore: \n");
    for (index_t i = 0; i < gt_boxes_ignore.size(0); ++i) {
        printf("[");
        for (index_t j = 0; j < 5; ++j) {
            printf("%f, ", gt_boxes_ignore[i][j]);
        }
        printf("]\n");
    }
    */
  ...
  BBoxOverlap(all_rois, gt_boxes, IOUs, 0);
  // printf("start iof\n");
  // vector<index_t> ig_indexes;
  vector<DType> ig_max_overlaps(all_rois.size(0), 0.f);
  if (ignore_iof_thresh > 0 && gt_boxes_ignore.size(0) > 0) {
      TensorContainer<cpu, 2, DType> IOFs(Shape2(all_rois.size(0), gt_boxes_ignore.size(0)), 0.f);
      BBoxOverlap(all_rois, gt_boxes_ignore, IOFs, 1);
      for (index_t i = 0; i < IOFs.size(0); ++i) {
          DType max_value_ignore = IOFs[i][0];
          for (index_t j = 1; j < IOFs.size(1); ++j) {
              if (max_value_ignore < IOFs[i][j]) {
                  max_value_ignore = IOFs[i][j];
              }
          }
          ig_max_overlaps[i] = max_value_ignore;
          /*
          if (max_value_ignore > ignore_iof_thresh) {
              ig_indexes.push_back(i);
          }
          */
      }
  }
  /*
  printf("ig_indexes: ");
  for (index_t i = 0; i < ig_indexes.size(); ++i) {
      printf("%d ", ig_indexes[i]);
  }
  printf("\n");
  */

  // printf("finish iof\n");
  // printf("start sorting iou\n");
  vector<DType> max_overlaps(all_rois.size(0), 0.f);
  vector<DType> all_labels(all_rois.size(0), 0.f);
  vector<index_t> gt_assignment(all_rois.size(0), 0);
  vector<DType> repgt_max_overlaps(all_rois.size(0), 0.f);
  vector<DType> repgt_all_labels(all_rois.size(0), 0.f);
  vector<index_t> repgt_assignment(all_rois.size(0), 0);
  ...
  } else if (gt_boxes.size(0) > 1) {
      for (index_t i = 0; i < IOUs.size(0); ++i) {
          DType max_value = IOUs[i][0];
          index_t max_index = 0;
          DType sec_max_value = IOUs[i][1];
          index_t sec_max_index = 1;
          if (IOUs[i][0] < IOUs[i][1]) {
              max_value = IOUs[i][1];
              max_index = 1;
              sec_max_value = IOUs[i][0];
              sec_max_index = 0;
          }
          for (index_t j = 2; j < IOUs.size(1); ++j) {
            if (max_value < IOUs[i][j]) {
                sec_max_value = max_value;
                sec_max_index = max_index;
                max_value = IOUs[i][j];
                max_index = j;
            } else if(sec_max_value < IOUs[i][j]) {
                sec_max_value = IOUs[i][j];
                sec_max_index = j;
            }
          }
          gt_assignment[i] = max_index;
          max_overlaps[i] = max_value;
          all_labels[i] = gt_boxes[max_index][4];
          repgt_assignment[i] = sec_max_index;
          repgt_max_overlaps[i] = sec_max_value;
          repgt_all_labels[i] = gt_boxes[sec_max_index][4];
      }
  }
  /*
  ...
  */
  ...
  for (index_t i = 0; i < max_overlaps.size(); ++i) {
    ...
    } else if (max_overlaps[i] > 0.f || ig_max_overlaps[i] < ignore_iof_thresh) {
        neg_indexes.push_back(i);
    }
  }
  /*
  printf("size of rois: %d, gt_ig: %d.\n", static_cast<int>(all_rois.size(0)), static_cast<int>(gt_boxes_ignore.size(0)));
  printf("size of gt: %d, fg: %d, neg: %d.\n", static_cast<int>(gt_boxes.size(0)), static_cast<int>(fg_indexes.size()), static_cast<int>(neg_indexes.size()));
  */
  // subsampling rois
  ...
  */
  ...
  for (index_t i = 0; i < max_overlaps.size(); ++i) {
    if ((max_overlaps[i] > bg_thresh_lo || ig_max_overlaps[i] < ignore_iof_thresh) && max_overlaps[i] < bg_thresh_hi) {
        bg_indexes.push_back(i);
    }
  }
  // printf("size of bg: %d\n", static_cast<int>(bg_indexes.size()));
  ...
  */
  /*
  printf("size of kept_indexes: %d\n", static_cast<int>(kept_indexes.size()));
  for (index_t i = 0; i < kept_indexes.size(); ++i) {
      printf("%d ", kept_indexes[i]);
  }
  printf("\n");
  */
  ...
  for (index_t i = 0; i < kept_indexes.size(); ++i) {
    if (i < fg_rois_this_image){
      ...
      if (repgt_all_labels[kept_indexes[i]] > static_cast<DType>(0)) {
          if (repgt_max_overlaps[kept_indexes[i]] > fg_thresh) {
              repgt_weights[i] = static_cast<DType>(1);
          } else {
              repgt_weights[i] = static_cast<DType>(0);
          }
      } else {
          repgt_weights[i] = static_cast<DType>(0);
      }
    ...
    Copy(repgt_rois[i], gt_boxes[repgt_assignment[kept_indexes[i]]].Slice(0, 4));
    match_gt_ious[i] = max_overlaps[kept_indexes[i]];
  }
  // printf("finish repgt_weights\n");
...
//        bbox_target_data[i][0] = min(1.f, labels[i]);
  ...
  // printf("start repbox\n");
  TensorContainer<cpu, 2, DType> gt_IOUs(Shape2(gt_boxes.size(0), gt_boxes.size(0)), 0.f);
  BBoxOverlap(gt_boxes, gt_boxes, gt_IOUs, 0);

  for (index_t i = 0; i < kept_indexes.size(); ++i) {
      for (index_t j = 0; j < kept_indexes.size(); ++j) {
          if (labels[i] > static_cast<DType>(0) && labels[j] > static_cast<DType>(0)) {
              if (gt_assignment[kept_indexes[i]] != gt_assignment[kept_indexes[j]]) {
                  repbox_weights[i][j] = gt_IOUs[gt_assignment[kept_indexes[i]]][gt_assignment[kept_indexes[j]]];
              } else {
                  repbox_weights[i][j] = static_cast<DType>(1);
              }
          } else {
              repbox_weights[i][j] = static_cast<DType>(1);
          }
...
template<>
Operator *CreateOp<cpu>(ProposalTargetParam_v3 param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ProposalTargetOp_v3<cpu, DType>(param);
  })
  return op;
}

template<>
Operator *CreateOp<gpu>(ProposalTargetParam_v3 param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ProposalTargetOp_v3<gpu, DType>(param);
  })
  return op;
}

Operator *ProposalTargetProp_v3::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(ProposalTargetParam_v3);

MXNET_REGISTER_OP_PROPERTY(ProposalTarget_v3, ProposalTargetProp_v3)
.describe("C++ version proposal target v3")
.add_argument("rois", "Symbol", "rois")
.add_argument("gt_boxes", "Symbol", "gtboxes")
.add_argument("gt_boxes_ignore", "Symbol", "gtboxesignore")
.add_arguments(ProposalTargetParam_v3::__FIELDS__());
