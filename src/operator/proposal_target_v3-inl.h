 ...
 * \file proposal_target_v3-inl.h
 * \brief C++ version proposal target v3
 * \author Yuntao Chen, Ziyang Zhou, Bo Ke
 */
#ifndef MXNET_OPERATOR_PROPOSAL_TARGET_V3_INL_H_
#define MXNET_OPERATOR_PROPOSAL_TARGET_V3_INL_H_
...
inline void SampleROI(
  ...
  Tensor<cpu, 2, DType> &&repgt_rois,
  Tensor<cpu, 1, DType> &&repgt_weights,
  Tensor<cpu, 2, DType> &&repbox_weights,
...
namespace proposal_target_v3_enum {
enum ProposalTargetInputs {kRois, kGtBboxes, kGtBboxesIgnore};
enum ProposalTargetOutputs {kRoiOutput, kLabel, kBboxTarget, kBboxWeight, kRepgtRois, kRepgtWeight, kRepboxWeight, kMatch_gt_iou};
}

struct ProposalTargetParam_v3 : public dmlc::Parameter<ProposalTargetParam_v3> {
  index_t num_classes;
  ...
  DMLC_DECLARE_PARAMETER(ProposalTargetParam_v3) {
    ...
    DMLC_DECLARE_FIELD(ignore_iof_thresh).describe("Ignore bounding boxes IOF threshold");
...
class ProposalTargetOp_v3 : public Operator {
 public:
  explicit ProposalTargetOp_v3(ProposalTargetParam_v3 param) {
    this->param_ = param;
  }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    CHECK_EQ(out_data.size(), 8);
    CHECK_GT(req.size(), 4);
    CHECK_EQ(req[proposal_target_v3_enum::kRoiOutput], kWriteTo);
    CHECK_EQ(req[proposal_target_v3_enum::kLabel], kWriteTo);
    CHECK_EQ(req[proposal_target_v3_enum::kBboxTarget], kWriteTo);
    CHECK_EQ(req[proposal_target_v3_enum::kBboxWeight], kWriteTo);
    CHECK_EQ(req[proposal_target_v3_enum::kRepgtRois], kWriteTo);
    CHECK_EQ(req[proposal_target_v3_enum::kRepgtWeight], kWriteTo);
    CHECK_EQ(req[proposal_target_v3_enum::kRepboxWeight], kWriteTo);
 //   printf("PROPOSAL_TARGET START\n");
    Stream<xpu> *s = ctx.get_stream<xpu>();
    const index_t num_image         = param_.batch_images;
    const index_t num_roi           = in_data[proposal_target_v3_enum::kRois].Size() / (num_image * 4);
    const index_t num_gtbbox        = in_data[proposal_target_v3_enum::kGtBboxes].Size() / (num_image * 5);
    const index_t num_gtbbox_ignore = in_data[proposal_target_v3_enum::kGtBboxesIgnore].Size() / (num_image * 5);
    const int image_rois            = param_.image_rois;
    Tensor<xpu, 3, DType> xpu_rois             = in_data[proposal_target_v3_enum::kRois].
                                                 get_with_shape<xpu, 3, DType>(Shape3(num_image, num_roi, 4), s);
    Tensor<xpu, 3, DType> xpu_gt_bboxes        = in_data[proposal_target_v3_enum::kGtBboxes].
                                                 get_with_shape<xpu, 3, DType>(Shape3(num_image, num_gtbbox, 5), s);
    Tensor<xpu, 3, DType> xpu_gt_bboxes_ignore = in_data[proposal_target_v3_enum::kGtBboxesIgnore].
                                                 get_with_shape<xpu, 3, DType>(Shape3(num_image, num_gtbbox_ignore, 5), s);
    TensorContainer<cpu, 3, DType> rois     (xpu_rois.shape_);
    TensorContainer<cpu, 3, DType> gt_bboxes(xpu_gt_bboxes.shape_);
    TensorContainer<cpu, 3, DType> gt_bboxes_ignore(xpu_gt_bboxes_ignore.shape_);
    Copy(rois, xpu_rois, s);
    Copy(gt_bboxes, xpu_gt_bboxes, s);
    Copy(gt_bboxes_ignore, xpu_gt_bboxes_ignore, s);

    std::vector<std::vector<Tensor<cpu, 1, DType>>> kept_rois;
    std::vector<std::vector<Tensor<cpu, 1, DType>>> kept_gtbboxes;
    std::vector<std::vector<Tensor<cpu, 1, DType>>> kept_gtbboxes_ignore;
    ...
    for (index_t i = 0; i < num_image; ++i) {
        ...
        if (kept_gtbboxes[i].empty()) {
            LOG(FATAL) << "Groundtruth is Empty.";
        }
    }
    for (index_t i = 0; i < num_image; ++i) {
        kept_gtbboxes_ignore.push_back(std::vector<Tensor<cpu, 1, DType>>());
        for (index_t j = 0; j < gt_bboxes_ignore.size(1); ++j) {
            if (gt_bboxes_ignore[i][j][4] != -1) {
                kept_gtbboxes_ignore[i].push_back(gt_bboxes_ignore[i][j]);
            }
        }
    }
  ...
  //  printf("PROPOSAL_TARGET MID\n");
    ...
    TensorContainer<cpu, 3, DType> cpu_repgt_rois(Shape3(num_image, image_rois, 4), 0.f);
    TensorContainer<cpu, 2, DType> cpu_repgt_weights(Shape2(num_image, image_rois), 0.f);
    TensorContainer<cpu, 3, DType> cpu_repbox_weights(Shape3(num_image, image_rois, image_rois), 0.f);
    ...
    if (param_.ohem) {
        ...
        for (index_t i = 0; i < num_image; ++i) {
          ...
          TensorContainer<cpu, 2, DType> kept_gtbboxes_ignore_i(Shape2(kept_gtbboxes_ignore[i].size(), gt_bboxes_ignore.size(2)));
          ...
          for (index_t j = 0; j < kept_gtbboxes_ignore_i.size(0); ++j) {
              Copy(kept_gtbboxes_ignore_i[j], kept_gtbboxes_ignore[i][j]);
          }

      //    printf("BEFORE SAMPLEROI");
          proposal_target_v3::SampleROI(
            kept_rois_i,
            kept_gtbboxes_i,
            kept_gtbboxes_ignore_i,
            bbox_mean,
            bbox_std,
            bbox_weight,
            fg_rois_per_image,
            ...
            cpu_repgt_rois[i],
            cpu_repgt_weights[i],
            cpu_repbox_weights[i],
            cpu_match_gt_ious[i]
          );
        }
    }

    Tensor<xpu, 3, DType> xpu_output_rois  = out_data[proposal_target_v3_enum::kRoiOutput].
                                             get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, 4), s);
    Tensor<xpu, 2, DType> xpu_labels       = out_data[proposal_target_v3_enum::kLabel].
                                             get_with_shape<xpu, 2, DType>(Shape2(num_image, image_rois), s);
    Tensor<xpu, 3, DType> xpu_bbox_targets = out_data[proposal_target_v3_enum::kBboxTarget].
                                             get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, param_.num_classes * 4), s);
    Tensor<xpu, 3, DType> xpu_bbox_weights = out_data[proposal_target_v3_enum::kBboxWeight].
                                             get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, param_.num_classes * 4), s);
    Tensor<xpu, 3, DType> xpu_repgt_rois   = out_data[proposal_target_v3_enum::kRepgtRois].
                                             get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, 4), s);
    Tensor<xpu, 2, DType> xpu_repgt_weights = out_data[proposal_target_v3_enum::kRepgtWeight].
                                              get_with_shape<xpu, 2, DType>(Shape2(num_image, image_rois), s);
    Tensor<xpu, 3, DType> xpu_repbox_weights = out_data[proposal_target_v3_enum::kRepboxWeight].
                                               get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, image_rois), s);
    Tensor<xpu, 2, DType> xpu_match_gt_ious = out_data[proposal_target_v3_enum::kMatch_gt_iou].
                                              get_with_shape<xpu, 2, DType>(Shape2(num_image, image_rois), s);
    ...
    Copy(xpu_repgt_rois, cpu_repgt_rois, s);
    Copy(xpu_repgt_weights, cpu_repgt_weights, s);
    Copy(xpu_repbox_weights, cpu_repbox_weights, s);
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    Stream<xpu> *s = ctx.get_stream<xpu>();
    Tensor<xpu, 3, DType> rois             = in_grad[proposal_target_v3_enum::kRois].get<xpu, 3, DType>(s);
    Tensor<xpu, 3, DType> gt_bboxes        = in_grad[proposal_target_v3_enum::kGtBboxes].get_with_shape<xpu, 3, DType>(Shape3(num_image, num_gtbbox, 5), s);
    Tensor<xpu, 3, DType> gt_bboxes_ignore = in_grad[proposal_target_v3_enum::kGtBboxesIgnore].get_with_shape<xpu, 3, DType>(Shape3(num_image, num_gtbbox_ignore, 5), s);

    rois = 0.f;
    gt_bboxes = 0.f;
    gt_bboxes_ignore = 0.f;
  }

 private:
  ProposalTargetParam_v3 param_;
};  // class ProposalTargetOp_v3

template<typename xpu>
Operator *CreateOp(ProposalTargetParam_v3 param, int dtype);

#if DMLC_USE_CXX11
class ProposalTargetProp_v3 : public OperatorProperty {
 public:
  ...
  std::vector<std::string> ListArguments() const override {
    return {"rois", "gt_boxes", "gt_boxes_ignore"};
  }

  std::vector<std::string> ListOutputs() const override {
    return {"roi_output", "label", "bbox_target", "bbox_weight", "repgt_rois", "repgt_weights", "repbox_weights", "match_gt_iou"};
  }
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    using namespace mshadow;
    CHECK_EQ(in_shape->size(), 3) << "Input: [rois, gt_boxes, gt_boxes_ignore]";
    const TShape &dshape = in_shape->at(proposal_target_v3_enum::kRois);
    const int image_rois  = param_.image_rois;
    ...
    auto repgt_rois_shape = Shape3(dshape[0], image_rois, 4);
    auto repgt_weight_shape = Shape2(dshape[0], image_rois);
    auto repbox_weight_shape = Shape3(dshape[0], image_rois, image_rois);
    ...
    out_shape->push_back(repgt_rois_shape);
    out_shape->push_back(repgt_weight_shape);
    out_shape->push_back(repbox_weight_shape);
  ...
  std::string TypeString() const override {
    return "ProposalTarget_v3";
  }

  OperatorProperty *Copy() const override {
    auto ptr = new ProposalTargetProp_v3();
    ptr->param_ = param_;
    return ptr;
  }
 ...
 private:
  ProposalTargetParam_v3 param_;
};  // class ProposalTargetProp_v3
#endif  // DMLC_USE_CXX11

}  // namespace op
}  // namespace mxnet

#endif  // MXNET_OPERATOR_PROPOSAL_TARGET_v3_INL_H_
