...
class ProposalOp_v1 : public Operator{
 public:
  explicit ProposalOp_v1(ProposalParam_v1 param) {
    this->param_ = param;
  }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    CHECK_GT(req.size(), 1);
    CHECK_EQ(req[proposal_v1::kOut], kWriteTo);

    Stream<xpu> *s = ctx.get_stream<xpu>();

    // Shape<3> fg_scores_shape = Shape3(in_data[proposal_v1::kClsProb].shape_[1] / 2,
    //                                   in_data[proposal_v1::kClsProb].shape_[2],
    //                                   in_data[proposal_v1::kClsProb].shape_[3]);

    // real_t* foreground_score_ptr = in_data[proposal_v1::kClsProb].dptr<real_t>()
    //                                 + fg_scores_shape.Size();
    Tensor<xpu, 4> scores = in_data[proposal_v1::kClsProb].get<cpu, 4, real_t>(s);
    Tensor<cpu, 4> bbox_deltas = in_data[proposal_v1::kBBoxPred].get<cpu, 4, real_t>(s);
    Tensor<cpu, 2> im_info = in_data[proposal_v1::kImInfo].get<cpu, 2, real_t>(s);

    Tensor<cpu, 3> out = out_data[proposal_v1::kOut].get<cpu, 3, real_t>(s);
    Tensor<cpu, 3> out_score = out_data[proposal_v1::kScore].get<cpu, 3, real_t>(s);
    ...
    Tensor<cpu, 1> workspace = ctx.requested[proposal_v1::kTempSpace].get_space<cpu>(
    ...
    proposal_v1_utils::GenerateAnchors(base_anchor,
                                    param_.ratios.info,
                                    param_.scales.info,
                                    &anchors);
    for(int n = 0; n < nbatch; n++) {
    ...
    if (param_.iou_loss) {
      proposal_v1_utils::IoUTransformInv(workspace_proposals, bbox_deltas, im_info, param_.feature_stride,
                                         &(workspace_proposals));
    } else {
      proposal_v1_utils::BBoxTransformInv(workspace_proposals, bbox_deltas,im_info, param_.feature_stride,
                                          &(workspace_proposals));
    }
    proposal_v1_utils::FilterBox(&workspace_proposals, param_.rpn_min_size, im_info);

    Tensor<cpu, 2> score = workspace_pre_nms[0];
    Tensor<cpu, 2> order = workspace_pre_nms[1];

    proposal_v1_utils::CopyScore(workspace_proposals,
                                 &score,
                                 &order);
    ...
    for(int n = 0; n < nbatch; n++) {
      ...
      proposal_v1_utils::ReverseArgsort(score[n],
                                        &cur_order);
      proposal_v1_utils::ReorderProposals(workspace_proposals[n],
                                          cur_order,
                                          rpn_pre_nms_top_n,
      ...
      proposal_v1_utils::NonMaximumSuppression(cur_workspace_ordered_proposals,
                                               param_.threshold,
                                               rpn_post_nms_top_n,
                                               &cur_area,
                                               &cur_suppressed,
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    Stream<xpu> *s = ctx.get_stream<xpu>();
    Tensor<xpu, 4> gscores = in_grad[proposal_v1::kClsProb].get<xpu, 4, real_t>(s);
    Tensor<xpu, 4> gbbox = in_grad[proposal_v1::kBBoxPred].get<xpu, 4, real_t>(s);
    Tensor<xpu, 2> ginfo = in_grad[proposal_v1::kImInfo].get<xpu, 2, real_t>(s);

    // can not assume the grad would be zero
    Assign(gscores, req[proposal_v1::kClsProb], 0);
    Assign(gbbox, req[proposal_v1::kBBoxPred], 0);
    Assign(ginfo, req[proposal_v1::kImInfo], 0);
  }

 private:
  ProposalParam_v1 param_;
};  // class ProposalOp_v1

template<>
Operator *CreateOp<cpu>(ProposalParam_v1 param) {
  return new ProposalOp_v1<cpu>(param);
}

Operator* ProposalProp_v1::CreateOperator(Context ctx) const {
  DO_BIND_DISPATCH(CreateOp, param_);
}

DMLC_REGISTER_PARAMETER(ProposalParam_v1);

MXNET_REGISTER_OP_PROPERTY(_contrib_Proposal_v1, ProposalProp_v1)
...
.add_arguments(ProposalParam_v1::__FIELDS__());
