...
class ProposalGPUOp_v1 : public Operator{
 public:
  explicit ProposalGPUOp_v1(ProposalParam_v1 param) {
    this->param_ = param;
  }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    Tensor<xpu, 4> scores = in_data[proposal_v1::kClsProb].get<xpu, 4, float>(s); // batch_idx, anchor_idx, height_idx, width_idx
    Tensor<xpu, 4> bbox_deltas = in_data[proposal_v1::kBBoxPred].get<xpu, 4, float>(s); // batch_idx, height_idx, width_idx, anchor_idx
    Tensor<xpu, 2> im_info = in_data[proposal_v1::kImInfo].get<xpu, 2, float>(s); // batch_idx, 3(height, width, scale)

    Tensor<xpu, 3> out = out_data[proposal_v1::kOut].get<xpu, 3, float>(s); // batch_idx, rois_idx, 4(x1, y1, x2, y2), batch_idx is needed after flatten
    Tensor<xpu, 3> out_score = out_data[proposal_v1::kScore].get<xpu, 3, float>(s); // batch_idx, rois_idx, 1(score)

    uint64_t WORKSPACE_LIMIT = 1024 * 1024 * param_.workspace; // 256 MB should be sufficient
    Tensor<xpu, 1, uint8_t> workspace = ctx.requested[proposal_v1::kTempSpace].get_space_typed<xpu, 1, uint8_t>(Shape1(WORKSPACE_LIMIT), s);
    ...
    for (int i = 0; i < nbatch; i++) {
      ...
      Tensor<cpu, 1, uint64_t> mask_host_tensor = ctx.requested[proposal_v1::kTempSpace].get_host_space_typed<1, uint64_t>(Shape1(boxes_num * col_blocks));
...
};  // class ProposalGPUOp_v1

template<>
Operator* CreateOp<gpu>(ProposalParam_v1 param) {
  return new ProposalGPUOp_v1<gpu>(param);
}
}  // namespace op
}  // namespace mxnet
