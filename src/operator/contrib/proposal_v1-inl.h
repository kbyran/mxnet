...
/*!
 * \file proposal_v1-inl.h
 * \brief Proposal_v1 Operator
 * \author Piotr Teterwak, Bing Xu, Jian Guo, Pengfei Chen, Yuntao Chen
*/
#ifndef MXNET_OPERATOR_CONTRIB_PROPOSAL_V1_INL_H_
#define MXNET_OPERATOR_CONTRIB_PROPOSAL_V1_INL_H_
...
struct ProposalParam_v1 : public dmlc::Parameter<ProposalParam_v1> {
  ...
  DMLC_DECLARE_PARAMETER(ProposalParam_v1) {
    float tmp[] = {0, 0, 0, 0};
...
Operator *CreateOp(ProposalParam_v1 param);

#if DMLC_USE_CXX11
class ProposalProp_v1 : public OperatorProperty {
 public:
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    const TShape &dshape = in_shape->at(proposal_v1::kClsProb);
    if (dshape.ndim() == 0) return false;
    Shape<4> bbox_pred_shape;
    bbox_pred_shape = Shape4(dshape[0], dshape[1] * 2, dshape[2], dshape[3]);
    SHAPE_ASSIGN_CHECK(*in_shape, proposal_v1::kBBoxPred,
                       bbox_pred_shape);
    Shape<2> im_info_shape;
    im_info_shape = Shape2(dshape[0], 3);
    SHAPE_ASSIGN_CHECK(*in_shape, proposal_v1::kImInfo, im_info_shape);
    out_shape->clear();
    // output
  ...
  OperatorProperty* Copy() const override {
    auto ptr = new ProposalProp_v1();
    ptr->param_ = param_;
    return ptr;
  }

  std::string TypeString() const override {
    return "_contrib_Proposal_v1";
  }
...
#endif  //  MXNET_OPERATOR_CONTRIB_PROPOSAL_V1_INL_H_
