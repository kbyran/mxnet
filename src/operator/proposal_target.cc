...
inline void SampleROI(const Tensor<cpu, 2, DType> &all_rois,
  ...
  */
  ...
  /*
  printf("size of ig: %d\n", static_cast<int>(ig_indexes.size()));
  printf("size of gt: %d, gt_ig: %d, fg: %d, bg: %d, neg: %d.\n", static_cast<int>(gt_boxes.size(0)), static_cast<int>(gt_boxes_ignore.size(0)), static_cast<int>(fg_indexes.size()), static_cast<int>(bg_indexes.size()), static_cast<int>(neg_indexes.size()));
  printf("size of bg for sample: %d\n", static_cast<int>(bg_indexes.size()));
  */
  ...
  */
  /*
  printf("size of kept_indexes: %d: ", static_cast<int>(kept_indexes.size()));
  ...
  */
  for (index_t i = 0; i < kept_indexes.size(); ++i) {
    if (i < fg_rois_this_image){
        /*
        if (kept_indexes[i] >= static_cast<int>(all_labels.size())) {
            printf("labels out of index: %d, %d\n", kept_indexes[i], static_cast<int>(all_labels.size()));
        }
        */
      labels[i] = all_labels[kept_indexes[i]];
    }
    /*
    if (kept_indexes[i] >= static_cast<int>(all_rois.size(0))) {
        printf("rois out of index: %d, %d\n", kept_indexes[i], static_cast<int>(all_rois.size(0)));
    }
    */
